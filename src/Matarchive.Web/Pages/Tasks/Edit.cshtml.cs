...
public sealed class EditModel : AppPageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(Guid id)
    {
        ...
        DestinationConnectionId = task.DestinationConnectionId;
        CompressToZip = task.CompressToZip;
        ArchiveFormat = TaskOptionCatalog.NormalizeArchiveFormat(task.ArchiveFormat, task.CompressToZip);
        CompressionLevel = TaskOptionCatalog.NormalizeCompressionLevel(task.CompressionLevel);
        TransferMode = TaskOptionCatalog.NormalizeTransferMode(task.TransferMode);
        VerifyDestination = task.VerifyDestination;
        KeepLocalStagingOnFailure = task.KeepLocalStagingOnFailure;
        ArchiveFileNamePattern = string.IsNullOrWhiteSpace(task.ArchiveFileNamePattern)
            ? MatarchiveConstants.DefaultArchiveFileNamePattern
            : task.ArchiveFileNamePattern;
        Enabled = task.Enabled;
        RunEveryMinutes = task.RunEveryMinutes;
        ScheduleMode = TaskSchedulePolicy.NormalizeScheduleMode(task.ScheduleMode, task.RunEveryMinutes);
        ScheduleIntervalMinutes = task.ScheduleIntervalMinutes ?? task.RunEveryMinutes;
        ScheduleTime = TaskSchedulePolicy.NormalizeScheduleTime(task.ScheduleTime);
        SelectedScheduleDays = TaskSchedulePolicy.ParseScheduleDays(task.ScheduleDays)
            .OrderBy(day => ((int)day + 6) % 7)
            .Select(day => day.ToString())
            .ToArray();
        ScheduleTimeZoneId = string.IsNullOrWhiteSpace(task.ScheduleTimeZoneId) ? MatarchiveConstants.DefaultScheduleTimeZoneId : task.ScheduleTimeZoneId;
        RetentionMode = TaskRetentionPolicy.NormalizeRetentionMode(task.RetentionMode);
        RetentionKeepLast = task.RetentionKeepLast ?? 10;
        RetentionKeepDays = task.RetentionKeepDays ?? 30;
        Notes = task.Notes;
        await LoadOptionsAsync(SourceConnectionId, DestinationConnectionId);
        return Page();
    }

    public async Task<IActionResult> OnPostAsync()
    {
        var connections = await LoadOptionsAsync(SourceConnectionId, DestinationConnectionId);
        NormalizeOptions();
        ...
        task.SourceConnectionId = SourceConnectionId!.Value;
        task.DestinationConnectionId = DestinationConnectionId!.Value;
        task.CompressToZip = CompressToZip;
        task.ArchiveFormat = ArchiveFormat;
        task.CompressionLevel = CompressionLevel;
        task.TransferMode = TransferMode;
        task.VerifyDestination = VerifyDestination;
        task.KeepLocalStagingOnFailure = KeepLocalStagingOnFailure;
        task.ArchiveFileNamePattern = ArchiveFileNamePattern.Trim();
        task.Enabled = Enabled;
        task.RunEveryMinutes = ScheduleMode == "Interval" ? NormalizeInterval(ScheduleIntervalMinutes) : null;
        task.ScheduleMode = ScheduleMode;
        task.ScheduleIntervalMinutes = NormalizeInterval(ScheduleIntervalMinutes);
        task.ScheduleTime = TaskSchedulePolicy.NormalizeScheduleTime(ScheduleTime);
        task.ScheduleDays = TaskSchedulePolicy.NormalizeScheduleDays(SelectedScheduleDays);
        task.ScheduleTimeZoneId = string.IsNullOrWhiteSpace(ScheduleTimeZoneId) ? MatarchiveConstants.DefaultScheduleTimeZoneId : ScheduleTimeZoneId.Trim();
        task.RetentionMode = RetentionMode;
        task.RetentionKeepLast = TaskRetentionPolicy.NormalizePositive(RetentionKeepLast) ?? 10;
        task.RetentionKeepDays = TaskRetentionPolicy.NormalizePositive(RetentionKeepDays) ?? 30;
        task.Notes = Notes.Trim();
        task.UpdatedAt = DateTimeOffset.UtcNow;
        await _repository.SaveTaskAsync(task);

        FlashSuccess = $"Task {task.Name} wurde gespeichert.";
        return RedirectToPage("/Tasks/Index");
    }

    private async Task<List<ConnectionProfile>> LoadOptionsAsync(Guid? selectedSourceId = null, Guid? selectedDestinationId = null)
    {
        var connections = await _repository.GetConnectionsAsync();
        SourceConnectionOptions = connections
            .Where(connection => ConnectionTypeCatalog.CanRead(connection) || connection.Id == selectedSourceId)
        ...
        DestinationConnectionOptions = connections
            .Where(connection => ConnectionTypeCatalog.CanWrite(connection) || connection.Id == selectedDestinationId)
            .OrderBy(connection => connection.Name)
            .Select(connection => new SelectListItem(BuildConnectionOptionLabel(connection), connection.Id.ToString()))
            .ToList();
